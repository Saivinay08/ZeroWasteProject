...
app.post("/donat_food_submit", async function (req, res) {
  ...
  await donationHistoryRef.add({
    ...
    D_ph_no: donSnapshot.docs[0].data().ph_no,
...
app.get("/view_route", (req, res) => {
  const { org, don } = req.query;
  res.render("map_view", {
    orgAddress: org,
    donAddress: don,
  });
...
const donHistoryRef = donSnapshot.docs[0].ref.collection("Donation_History");
const donDataSnap = await donHistoryRef.where("OrderId", "==", orderid).get();

const donDoc = donDataSnap.docs[0];
...
const orgData = donorSnapshot.docs[0].data(); // Organization data
const donorData = donSnapshot.docs[0].data(); // Donor data

const orgAddress = `${orgData.street}, ${orgData.city}, ${orgData.dist}, ${orgData.state}, ${orgData.pincode}`;
const donAddress = `${donorData.street}, ${donorData.city}, ${donorData.dist}, ${donorData.state}, ${donorData.pincode}`;

res.redirect(`/view_route?org=${encodeURIComponent(orgAddress)}&don=${encodeURIComponent(donAddress)}`);
});
...
app.post("/donation_collect_route", async (req, res) => {
  const org_email = req.session.orgEmail;
  const donor_email = req.body.orderemail;
  const orderid = req.body.orderid;

  const orgQuery = db.collection("Organizations").where("email", "==", org_email);
  const orgSnapshot = await orgQuery.get();
  const orgDoc = orgSnapshot.docs[0];

  const orgHisRef = orgDoc.ref.collection("Donation_History").where("OrderId", "==", orderid);
  const orgHisSnapshot = await orgHisRef.get();
  await orgHisSnapshot.docs[0].ref.update({ Status: "Collected" });

  const donQuery = db.collection("Donors").where("email", "==", donor_email);
  const donSnapshot = await donQuery.get();
  const donDoc = donSnapshot.docs[0];

  const donHisRef = donDoc.ref.collection("Donation_History").where("OrderId", "==", orderid);
  const donHisSnapshot = await donHisRef.get();
  await donHisSnapshot.docs[0].ref.update({ Status: "Collected" });
  ...
  const orgAddress = `${orgData.street}, ${orgData.city}, ${orgData.dist}, ${orgData.state}, ${orgData.pincode}`;
  const donAddress = `${donData.street}, ${donData.city}, ${donData.dist}, ${donData.state}, ${donData.pincode}`;

  // ✅ Use backticks and quotes correctly
  res.redirect(`/view_route?org=${encodeURIComponent(orgAddress)}&don=${encodeURIComponent(donAddress)}`);
});
